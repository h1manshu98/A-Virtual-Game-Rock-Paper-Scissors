...
DATA_PATH = 'C:\\Users\\BHANU\\Desktop\\Projects College\\Internship IIIT Bhopal\\RockPaperScissor-AI--master' # Path to folder containing data
...
for dr in os.listdir(DATA_PATH):    #all directories inside the given path 'DATA_PATH
...
# zip() gives the address of ↑←imgData, by using '*' we can get the value at that address

imgData = np.asarray(imgData)   # gives the ARRAY of imgData's values
